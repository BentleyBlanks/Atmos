...
void t3Quaternion::get(t3Matrix4x4& matrix) const
{
...
void t3Quaternion::makeRotate(float angle, float x, float y, float z)
{
    angle = t3Math::Rad2Deg(angle);
...
void t3Quaternion::makeRotate(float angle1, const t3Vector3f& axis1,
                              float angle2, const t3Vector3f& axis2,
                              float angle3, const t3Vector3f& axis3)
{
    t3Quaternion q1; q1.makeRotate(angle1, axis1);
    t3Quaternion q2; q2.makeRotate(angle2, axis2);
    t3Quaternion q3; q3.makeRotate(angle3, axis3);
...
*/
void t3Quaternion::makeRotate(const t3Vector3f& from, const t3Vector3f& to)
{
    ...
    t3Vector3f sourceVector = from;
    t3Vector3f targetVector = to;
    ...
    else
    {
        ...
        const t3Vector3f tmp = sourceVector.getCrossed(targetVector) / (2.0 * s);
        _v.x = tmp.x;
        _v.y = tmp.y;
        _v.z = tmp.z;
        _v.w = s;
...
// with a rotation!
void t3Quaternion::getRotate(float& angle, float& x, float& y, float& z) const
{
...
void t3Quaternion::slerp(float t, const t3Quaternion& from, const t3Quaternion& to)
{
    const double epsilon = 0.00001;
    double omega, cosomega, sinomega, scale_from, scale_to;

    t3Quaternion quatTo(to);
...
t3Vector3f t3Quaternion::getEuler() const
{
    float test = x()*y() + z()*w();
    float heading;
    float attitude;
    ...
    return t3Vector3f(t3Math::Rad2Deg(bank), t3Math::Rad2Deg(heading), t3Math::Rad2Deg(attitude));
}
